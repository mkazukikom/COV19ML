...
comparison_results <- data_filtered %>%
  filter(CLASS != "HC") %>%
  group_by(CLASS) %>%
  summarise(p_value = wilcox.test(hc_data, BCORP1)$p.value)
...
ggplot(data_filtered, aes(x = class_order, y = BCORP1, fill = class_order)) +
  ...
  labs(title = "Anti-BCORP1", x = "Class", y = "[AU]") +
  geom_hline(yintercept = 2.258800045, linetype = 2, color = 'red') +
  theme_minimal() +
  geom_text(data = comparison_results, aes(x = CLASS, y = max(data_filtered$BCORP1) * 1.1, label = label), inherit.aes = FALSE)
...
# Generate ROC curve
roc_result <- roc(data_roc$COVID_status, data_roc$BCORP1)

# Create ROC plot
roc_plot <- ggroc(roc_result) +
  labs(
    title = "ROC Curve for anti-BCORP1",
    x = "Specificity",
    y = "Sensitivity"
  ) +
  theme_minimal()
