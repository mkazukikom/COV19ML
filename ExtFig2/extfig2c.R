...
# Plotting
ggplot(data_filtered, aes(x = class_order, y = log10(IFNA2), fill = class_order)) +
  geom_boxplot() +
  scale_fill_manual(values = class_colors) +
  scale_x_discrete(name = "Class") +
  labs(title = "Anti-IFNA2", x = "Class", y = "[Log10(AU)]") +
#  geom_hline(yintercept=0.562229191,linetype=2, color='red') +
  theme_minimal()
