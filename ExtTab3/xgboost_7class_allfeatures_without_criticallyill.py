...
data = data[data.CLASS != 'COVID_severe']
...
for (tr, te) in skf.split(X_np, y_all):
    ...
    evallist_all = [(dtrain_all, 'train')]
    bst = xgb.train(param, dtrain_all, n_round, evallist_all)

    pred_tr_proba = bst.predict(dtrain_all)
    ...
    pred_te_proba = bst.predict(dtest_all)
    ###
    pred_tr = pred_tr_proba.argmax(axis=1)
    pred_te = pred_te_proba.argmax(axis=1)
    ...
    auc_tr = roc_auc_score(y_tr, pred_tr_proba, multi_class='ovr', average='macro')
    auc_te = roc_auc_score(y_te, pred_te_proba, multi_class='ovr', average='macro')
