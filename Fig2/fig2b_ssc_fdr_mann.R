...
columns_to_analyze <- colnames(ssc_data)[which(colnames(ssc_data) == "A4GALT"):length(colnames(ssc_data))]
...
for (column in columns_to_analyze) {
  ssc_values <- ssc_data[[column]]
  hc_values <- hc_data[[column]]
  
  # 正規性検定（Shapiro-Wilk検定）
  shapiro_ssc <- shapiro.test(ssc_values)$p.value
  shapiro_hc <- shapiro.test(hc_values)$p.value
  
  # 使用する統計検定を選択
  if (shapiro_ssc > 0.05 & shapiro_hc > 0.05) {
    # 正規性が満たされている場合：t検定を使用
    test_result <- t.test(ssc_values, hc_values, alternative = "two.sided", var.equal = FALSE)
    test_used <- "t-test"
  } else {
    # 正規性が満たされていない場合：Mann-Whitney U検定を使用
    test_result <- wilcox.test(ssc_values, hc_values, alternative = "two.sided")
    test_used <- "Mann-Whitney U test"
  }
  
  # Log2 Fold Changeを計算
  mean_ssc <- mean(ssc_values, na.rm = TRUE)
  mean_hc <- mean(hc_values, na.rm = TRUE)
  log2fc <- log2(mean_ssc / mean_hc)
...
write.csv(significant_genes, "significant_genes_ssc_fdr_mann.csv", row.names = FALSE)
...
ggplot(results_df, aes(x = Log2FC, y = -log10(FDR))) +
  ...
  labs(title = "SSc", x = "Log2 Fold Change", y = "-log10(FDR)") +
  xlim(c(-5, 10)) +
  ylim(c(0, 30)) +
  theme_minimal() +
